...
module.exports = async function handler(req, res) {
  try {
    if (req.method === 'POST') {
      const report = validateReport(typeof req.body === 'string' ? JSON.parse(req.body) : req.body || {});
      const rows = await supabase(TABLE, {
        method: 'POST',
        headers: { Prefer: 'return=representation' },
        body: JSON.stringify(report)
      });
      send(res, 201, { ok: true, id: rows?.[0]?.id });
      return;
    }
    ...
    if (req.method === 'GET') {
      ...
      const keyword = clean(req.query?.keyword, 80).toLowerCase();
    ...
    send(res, req.method === 'POST' ? 400 : 500, { error: error.message || '请求失败' });
  }
};
